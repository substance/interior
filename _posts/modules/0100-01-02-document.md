...
### Position nodes
    ...
    var opC = [
      "position", "content", {"nodes": ["t2", "h1", "t1"], "target": 0}
    ];
    doc.exec(opC);
...
As you can see there are two nodes registered, which can be directly accessed by their `id`. In order to reflect the order of our nodes we keep a view node `content` that has a `nodes` property.
...
To learn about the full API, please consult our [test suite](https://github.com/substance/document/blob/master/tests/001-document-manipulation.js). It should be easy to follow the examples.
