...
As you can see there are two nodes registered, which can be directly accessed by their `id`. In order to reflect the order of our nodes we keep an array of node id's in `lists.content`.
...
Like everything, adding a comment needs to be done by specifying an operation.
    ...
    ["insert", {"id": "comment:a", "type": "comment", "data": {"node": "section:1", "content": "Shouldn't you add exclamation mark here?"}}]
